...
class BinanceFuturesClient:
    def __init__(self, public_key: str, secret_key: str, testnet: bool, mode: str):
        if testnet:
    ...
    def _make_request(self, method: str, endpoint: str, data: typing.Dict):
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                logger.error("Error while making %s request to %s: %s (error code %s)",
                             method, endpoint, response, response.status_code)
            return None
            ...
            if data['e'] == "aggTrade":
                ...
                for key, strat in self.strategies.items():
                    if strat.contract.symbol == symbol:
                        ...
                        strat.check_trade(res) # CHECK CHEQUEO DE TRADES
    ...
    def subscribe_channel(self, contracts: typing.List[Contract], channel: str):
        ...
        lista = ""
        lista = " ".join([ contract.symbol for contract in contracts ])
        # for contract in contracts:
        #     lista += contract.symbol

        print(f"Suscribiendo {channel} - {len(contracts)}:: {lista}")
        ...
        if self.mode == 'cut':
            for i, contract in enumerate(contracts):
                if i < 100:
                    ...
                    logger.warning("WARNING Maximum limit subscription reached. Will not subscribe to channel %s || %s", contract.symbol, channel)
        else:
            for contract in contracts:
                data['params'].append(contract.symbol.lower() + "@" + channel)
        # =========== MODO END
